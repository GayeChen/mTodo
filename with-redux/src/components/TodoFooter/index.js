  ...
  render() {
    ...
    <div className="row text-center">
      ...
      <div className="col-xs-5">
        {
          btns.map((btn, index) => (
          <button key={index} className={`btn ${filterType !== btn.type ? 'btn-default' : 'btn-success'} btn-sm`}
      ...
      </div>
      {
        completedTodoCount ? (<div className="col-xs-3">
          <button className="btn btn-danger btn-sm" onClick={() => this.props.todoActions.clearCompleted()}>删除已完成
          </button>
        </div>) : null
      }
    </div>
